...
\toprule
\textbf{Group} & \textbf{Model} & \textbf{Base mean} & \textbf{128K prompt} & \textbf{Decode} & \textbf{Allocation} & \textbf{Train MFU} \\
& & (\%) & (s) & (ms/token) & (GiB) & (\%, 2K) \\
\midrule
...
\midrule
AdamW/direct & TDA hybrid & 40.6 & 6.46 & 6668.5 & 5.98 & 38.64 \\
\bottomrule
\end{tabular}}
\caption{Short-context and systems trade-offs on one L40S, one sequence and one timing sample. TDA uses full-prefix recomputation (TTFT and peak allocated memory); other rows use cached/recurrent serving (prefill and engine allocation). Attention allocation includes a preallocated KV pool. These backend and memory conventions preclude an architecture-only comparison. Appendix~\ref{appendix:systems} gives details.}
\label{tab:systems}
\end{table}
